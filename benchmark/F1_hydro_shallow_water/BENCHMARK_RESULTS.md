...
| Framework | fp64 (ms) | fp32 (ms) | Speedup |
|-----------|----------|----------|---------|
| Kokkos | 23.768 | 13.326 | **1.78x** |
| Taichi | 25.970 | 20.253 | 1.28x |
| Warp | 27.792 | 13.970 | **1.99x** |
...
- **TileLang**: pip v0.1.8 TVM FFI bug (Buffer construction fails)

## 7. Key Findings

1. **Kokkos is fastest** in both fp32 and fp64, beating hand-written CUDA by 15%
2. **Warp ≈ CUDA naive** in fp64 (27.8 vs 27.3ms) — Warp's earlier speed advantage was purely fp32
3. **fp32 gives 1.3-2.0x speedup** over fp64 (Warp benefits most: 1.99x)
4. **Triton is 8.7x slower** — precompute-all-select-one is catastrophic for branchy kernels
5. **Gather promotion has no benefit** on B200 (60MB L2 cache covers working set)
6. **Framework compilers > hand-written CUDA** — Kokkos/Taichi produce better optimized code
