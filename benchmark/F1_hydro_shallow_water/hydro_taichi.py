...
def run_real(steps=1, backend="cuda", mesh="default"):
    ...
    DT = 0.5 * min_side / (ti.sqrt(G * 2.0) + 1e-6)
    ...
    # Load real mesh data into fields
    NAC.from_numpy(m["NAC"])
    KLAS.from_numpy(m["KLAS"])
    SIDE.from_numpy(m["SIDE"])
    COSF.from_numpy(m["COSF"])
    SINF.from_numpy(m["SINF"])
    SLCOS.from_numpy(m["SLCOS"])
    SLSIN.from_numpy(m["SLSIN"])
    AREA.from_numpy(m["AREA"])
    ZBC.from_numpy(m["ZBC"])
    FNC.from_numpy(m["FNC"])
    H_pre.from_numpy(m["H"])
    U_pre.from_numpy(m["U"])
    V_pre.from_numpy(m["V"])
    Z_pre.from_numpy(m["Z"])
    W_pre.from_numpy(m["W"])

    # ------------------------------------------------------------------
    # Reuse the same kernel logic (defined via closure over CEL, fields)
    # ------------------------------------------------------------------
    @ti.func
    def QF_r(h: ti.f64, u: ti.f64, v: ti.f64) -> ti.types.vector(4, ti.f64):
        f0 = h * u
        return ti.Vector([f0, f0 * u, f0 * v, HALF_G * h * h])

    @ti.func
    def osher_r(QL: ti.types.vector(3, ti.f64),
                QR: ti.types.vector(3, ti.f64),
                FIL_in: ti.f64, H_pos: ti.f64) -> ti.types.vector(4, ti.f64):
        CR = ti.sqrt(G * QR[0])
        FIR_v = QR[1] - 2.0 * CR
        ...
        if K1 == 1:
            if K2 == 1:
                ...
                ua_ = (fil + fir) / 2.0; fil = fil - ua_; HA = fil * fil / (4.0 * G)
                FLR += QF_r(HA, ua_, QL[2])
            elif K2 == 3:
                ua_ = (fil + fir) / 2.0; fir = fir - ua_; HA = fir * fir / (4.0 * G)
                FLR += QF_r(HA, ua_, QR[2])
            else:
                US = fir / 3.0; HS = US * US / G
                FLR += QF_r(HS, US, QR[2])
        elif K1 == 2:
            if K2 == 1:
                FLR += QF_r(QL[0], QL[1], QL[2])
            elif K2 == 2:
                FLR += QF_r(QL[0], QL[1], QL[2])
                US2 = fil / 3.0; HS2 = US2 * US2 / G
                FLR -= QF_r(HS2, US2, QL[2])
                ua_ = (fil + fir) / 2.0; fil = fil - ua_; HA = fil * fil / (4.0 * G)
                FLR += QF_r(HA, ua_, QL[2])
            elif K2 == 3:
                FLR += QF_r(QL[0], QL[1], QL[2])
            ...
            for j in ti.static(range(1, 5)):
                ...
                if KP == 4:
                    ...
                    if pos < NC:
                        ...
                        QR_v_ = VC * COSJ - UC * SINJ
                        QR_vec = ti.Vector([QR_h, QR_u, QR_v_])
                        FLR_OS = osher_r(QL, QR_vec, FIL_v, H_pre[pos])
                        flux[0] = FLR_OS[0]
                        flux[1] = FLR_OS[1] + (1.0 - ratio) * HC * UR * UR / 2.0
                        flux[2] = FLR_OS[2]
                        flux[3] = FLR_OS[3]
                        ...
                        QR1_v_ = VI * COSJ1 - UI * SINJ1
                        QR1_vec = ti.Vector([QR1_h, QR1_u, QR1_v_])
                        FLR1 = osher_r(QL1, QR1_vec, FIL1, H_pre[NC])
                        flux[0] = -FLR1[0]
                        flux[1] = FLR1[1] + (1.0 - ratio1) * HC2 * UR1 * UR1 / 2.0
                        flux[2] = FLR1[2]
                        ZA = ti.sqrt(FLR1[3] / HALF_G) + BC
