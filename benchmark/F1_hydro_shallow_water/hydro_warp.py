...
# ---------------------------------------------------------------------------
@wp.func
def QF(h: float, u: float, v: float) -> wp.vec4:
    f0 = h * u
    return wp.vec4(f0, f0 * u, f0 * v, 4.905 * h * h)


@wp.func
def safe_copysign(val: float, sign_val: float) -> float:
    if sign_val >= 0.0:
        return wp.abs(val)
    else:
        return -wp.abs(val)
...
@wp.func
def osher_solver(QL_h: float, QL_u: float, QL_v: float,
                 QR_h: float, QR_u: float, QR_v: float,
                 FIL_in: float, H_pos: float) -> wp.vec4:
    ...
    if K1 == 1:
        if K2 == 1:  # QS<2>(+1)
            US = fil / 3.0
            HS = US * US / 9.81
            FLR = FLR + QF(HS, US, QL_v) * 1.0
        elif K2 == 2:  # QS<3>(+1)
            ua_ = (fil + fir) / 2.0
            fil = fil - ua_
            HA = fil * fil / (4.0 * 9.81)
            FLR = FLR + QF(HA, ua_, QL_v) * 1.0
        elif K2 == 3:  # QS<5>(+1)
            ua_ = (fil + fir) / 2.0
            fir = fir - ua_
            HA = fir * fir / (4.0 * 9.81)
            FLR = FLR + QF(HA, ua_, QR_v) * 1.0
        else:  # K2 == 4, QS<6>(+1)
            US = fir / 3.0
            HS = US * US / 9.81
            FLR = FLR + QF(HS, US, QR_v) * 1.0
    elif K1 == 2:
        if K2 == 1:  # QS<1>(+1)
            FLR = FLR + QF(QL_h, QL_u, QL_v) * 1.0
        elif K2 == 2:  # QS<1>(+1), QS<2>(-1), QS<3>(+1)
            FLR = FLR + QF(QL_h, QL_u, QL_v) * 1.0
            US2 = fil / 3.0
            HS2 = US2 * US2 / 9.81
            FLR = FLR + QF(HS2, US2, QL_v) * (-1.0)
            ua_ = (fil + fir) / 2.0
            fil = fil - ua_
            HA = fil * fil / (4.0 * 9.81)
            FLR = FLR + QF(HA, ua_, QL_v) * 1.0
...
def shallow_water_step(
    ...
    VI = V1
    if HI <= 0.01:
        UI = safe_copysign(0.001, UI)
        VI = safe_copysign(0.001, VI)
    ...
    for j in range(1, 5):
        ...
        if KP == 4 or (KP != 0):
            # Wall / other boundary
            flux3 = 4.905 * H1 * H1
        elif HI <= 0.001 and HC <= 0.001:
            pass
        elif ZI <= BC:
            flux0 = -0.3 * wp.pow(HC, 1.5)
            flux1 = HI * QL_u * wp.abs(QL_u)
            flux3 = 4.905 * HI * HI
        elif ZC <= BI:
            flux0 = 0.3 * wp.pow(HI, 1.5)
            flux1 = HI * wp.abs(QL_u) * QL_u
            flux2 = HI * wp.abs(QL_u) * QL_v
        elif HI <= 0.01:
            if ZC > ZI:
                DH = wp.max(ZC - ZBC[pos], 0.001)
                UN = -0.3 * wp.sqrt(DH)
                flux0 = DH * UN
                flux1 = flux0 * UN
            ...
            if pos < NC:
                QR_h = wp.max(ZC - ZBC[pos], 0.001)
                UR = UC * COSJ + VC * SINJ
                ratio = wp.min(HC / QR_h, 1.5)
                QR_u = UR * ratio
                if HC <= 0.01 or QR_h <= 0.01:
                    QR_u = safe_copysign(0.001, UR)
                ...
                CL1 = wp.sqrt(9.81 * H_pre[NC])
                FIL1 = QL1_u + 2.0 * CL1
                HC2 = wp.max(HI, 0.001)
                ZC1 = wp.max(ZBC[pos], ZI)
                QR1_h = wp.max(ZC1 - ZBC[NC], 0.001)
                UR1 = UI * COSJ1 + VI * SINJ1
                ratio1 = wp.min(HC2 / QR1_h, 1.5)
                QR1_u = UR1 * ratio1
                if HC2 <= 0.01 or QR1_h <= 0.01:
                    QR1_u = safe_copysign(0.001, UR1)
    ...
    if H2 > 0.001:
        if H2 <= 0.01:
            U2 = safe_copysign(wp.min(0.001, wp.abs(U1)), U1)
            V2 = safe_copysign(wp.min(0.001, wp.abs(V1)), V1)
        else:
            QX1 = H1 * U1
            QY1 = H1 * V1
            DTAU = WDTA * WU
            DTAV = WDTA * WV
            WSF = FNC[pos] * wp.sqrt(U1 * U1 + V1 * V1) / wp.pow(H1, 0.33333)
            U2 = (QX1 - DTAU - DT * WSF * U1) / H2
            V2 = (QY1 - DTAV - DT * WSF * V1) / H2
            if H2 > 0.01:
                U2 = safe_copysign(wp.min(wp.abs(U2), 15.0), U2)
                V2 = safe_copysign(wp.min(wp.abs(V2), 15.0), V2)
