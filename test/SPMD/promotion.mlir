...
// Negative: oversized tile footprint — promote-group-memory must skip
// promotion and emit a remark, but NOT insert any group alloc or barrier.
//
// 2-D stencil: reads A[i,j], A[i,j+1], A[i+1,j]  →  maxOffset=(1,1)
// tile_size = [128, 128],  step = 1,  extent = (128-1+1+1) × (128-1+1+1) = 129 × 129
// footprintBytes = 129 × 129 × 4 = 66,564 > maxGroupMemBytes (49,152)
//
// The reuse gate PASSES (maxOffset > minOffset in both dims), but the
// footprint gate rejects promotion and emits a remark to stderr.

// CHECK-LABEL: func @stencil2d_oversized
...
// Oversized footprint: no group alloc or barrier must appear in the output.
// CHECK-NOT: memref.alloc() : memref<{{.*}}#spmd.addr_space<group>>
// CHECK-NOT: spmd.barrier
